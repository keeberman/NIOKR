...
uses
  ...
  syncobjs, v_fastmod, frxClass, frxDesgn, Variants;
...
//Отчёты по ФастРепорту, добавлено 03.2016
    procedure frRN01_02Click(Sender: TObject);
    procedure RepPayment_1Click(Sender : TObject);
    procedure RepRN04Click(Sender : TObject);
    procedure RepPayment2Click(Sender: TObject);
    procedure RepRN06Click(Sender: TObject);
    procedure RepRN08Click(Sender: TObject);
    procedure RepRN10Click(Sender: TObject);
    procedure RepRN12Click(Sender: TObject);
    procedure RepRN13Click(Sender: TObject);
    procedure RepRN14Click(Sender: TObject);
    procedure RepRN16Click(Sender: TObject);
    procedure RepRN18Click(Sender: TObject);    
    procedure RepRN20Click(Sender: TObject);
    procedure RepRN22Click(Sender: TObject);
  private
//    TThread
...
begin
  ...
  if VarIsNull(dmglobal.quPrintLetter.GetVariable('V_EXEC_CURRENCY_NO'))
    or dmglobal.quPrintLetter.GetVariable('V_EXEC_CURRENCY_NO') = 974 then
  begin
  ...
  try
    ...
    try
      ...
      if (dmglobal.quPrintLetter.GetVariable('V_NDS_SUMMPAY') <> NULL) and
         (dmglobal.quPrintLetter.GetVariable('V_NDS_SUMMPAY') <> 0) then
      begin
        n := round(dmglobal.quPrintLetter.GetVariable('V_NDS_SUMMPAY'));
        i := 0;
        NDS_SUMMPAY := '';
        while(n <> 0) do
        begin
      ...
      begin
       ...
       if (not (   VarIsNull(dmglobal.quPrintLetter.GetVariable('V_EXEC_CURRENCY_NO'))
                or dmglobal.quPrintLetter.GetVariable('V_EXEC_CURRENCY_NO') = 974))
          and
...
//      // Проверяем код валюты, если не 974(руб. РБ) то выводим и курс и в другой столбец
////      if Integer(Field(4)) <> 974 then
////      begin
...
//    Excel.Worksheets[1].Range['$'+IntToStr(idx) + ':$'+IntToStr(idx+count_delete_row)].Delete;
   ...
   dmGlobal.frRN01_02.ShowPreparedReport;
    ...
    begin
      ...
      begin
        try
          ...
          dec(count_stages);
          if Integer(Field(6)) <> 974 then // Код валюты РБ.
    ...
    begin
      ...
      begin
        try
          ...
          dec(count_stages);
          if Integer(Field('CURRENCY_NO')) <> 974 then // Код валюты РБ.
          begin // Считаем для не РБ
          ...
          begin
            if summpay_per_year > 0 then
              Excel.WorkSheets[1].Cells[idx-2, 7] := IntToStr(Round(summpay_per_year))
            else
              Excel.WorkSheets[1].Cells[idx-2, 7] := '';

            if Integer(Field('CURRENCY_NO')) <> 974 then
            begin
              ...
              if Field('DATEFROM_YEAR') < '2004' then // Т.к. в БД нет всех данных до 2004 года,
                                                      // то значения графы в отчете ВЫП. В ПРЕДЫДУЩИЕ ГОДЫ
                                                      // для договоров заключенных раннее 2004 года считаем по особому
                summpay_per_treaty_prev_years := Round(summpay_per_treaty*erate+summpay_stages_closed -  summpay_per_treaty_this_next_years);
            end
            else
            begin
              Excel.WorkSheets[1].Cells[idx-2, 5] := IntToStr(Round(summpay_per_treaty));
            ...
            if summpay_per_treaty_prev_years > 0 then
              Excel.WorkSheets[1].Cells[idx-2, 6] := IntToStr(Round(summpay_per_treaty_prev_years))
            else
              Excel.WorkSheets[1].Cells[idx-2, 6] := '';
          end;
        except
      ...
      begin
        ...
        begin
          try
            ...
            if Integer(Field('currency_no')) <> 974 then // не беларуские рубли
              summpay_stage := Extended(Field('e_rate'))*Extended(Field('stage_summpay'))
            else
              summpay_stage := Extended(Field('stage_summpay'));

            Excel.WorkSheets[1].Cells[idx, 5] := IntToStr(Round(summpay_stage));
      ...
      begin
        ...
        begin
          try
            ...
            Excel.WorkSheets[1].Cells[idx, 3 + st_month] := IntToStr(Round(summpay_stage));
    ...
    begin
      ...
      begin
        try
          ...
          Excel.WorkSheets[1].Cells[idx, 6] := IntToStr(Round(summpay_stage));
...
const
  ...
  count_cols : integer = 9;  // Количество колонок
  offs_table : integer = 3; // Начало шаблона таблицы
  row_no_page_break : integer = 2; //Номер строки, где нет разрыва страницы
          ...
          try
            ...
            begin
              ...
              begin // Следующий договор
                ...
                if (Integer(Field('currency_no')) <> 974) or (FieldAsString('status') = 'F') then
                begin
                  if Integer(Field('currency_no')) <> 974 then
                    // иностранный договор
                    fr_tr := true;
                ...
                Excel.WorkSheets[1].Cells[idx, 4] := String(Field('exec_name')); // исполнитель
              ...
              begin
                ...
                begin
                  if not tr_closed then
                    Excel.WorkSheets[1].Cells[idx - 1, 7] := IntToStr(Round(summpay_per_treaty + summpay_stages_closed))
                                                           //Round((summpay_per_treaty + summpay_stages_closed)/100000)/10;
                  else
                    Excel.WorkSheets[1].Cells[idx - 2, 7] := IntToStr(Round(summpay_per_treaty + summpay_stages_closed));
                  if Round(summpay_per_treaty_year) > 0 then
                     //Round(summpay_per_treaty_year/100000)/10 > 0 then
                    if not tr_closed then
                      Excel.WorkSheets[1].Cells[idx - 1, 8] := IntToStr(Round(summpay_per_treaty_year))
                                                             //Round(summpay_per_treaty_year/100000)/10;
                    else
                      Excel.WorkSheets[1].Cells[idx - 2, 8] := IntToStr(Round(summpay_per_treaty_year));
                ...
                begin
                  Excel.WorkSheets[1].Cells[idx - 2, 7] := IntToStr(Round(summpay_per_treaty));
                  ...
                  if Round(summpay_per_treaty_year) > 0 then
                    ...
                    if I <> 2 then
                      ...
                      Excel.WorkSheets[1].Cells[idx - 2, 8] := IntToStr(Round(summpay_per_treaty) - Round(summpay_per_treaty_year));
                ...
                begin
                  if not tr_closed then
                    Excel.WorkSheets[1].Cells[idx - 1, 7] := IntToStr(Round(summpay_per_treaty))
                                                             //Round(summpay_per_treaty/100000)/10
                  else
                    Excel.WorkSheets[1].Cells[idx - 2, 7] := IntToStr(Round(summpay_per_treaty));
                                                             //Round(summpay_per_treaty/100000)/10;
        ...
        try
          ...
          begin
            ...
            if plan_summpay_per > 0 then
              Excel.WorkSheets[1].Cells[idx-2, 8] := IntToStr(Round(plan_summpay_per))
            else
              Excel.WorkSheets[1].Cells[idx-2, 8] := '';


            if fact_summpay_per > 0 then
              Excel.WorkSheets[1].Cells[idx-2, 9] := IntToStr(Round(fact_summpay_per))
            else
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Excel.WorkSheets[1].Cells[idx, 9] := IntToStr(Round(FieldAsFloat('MONTH_TREATY_SUMMPAY')));
            Excel.WorkSheets[1].Cells[idx, 10] := IntToStr(Round(FieldAsFloat('YEAR_TREATY_SUMMPAY')));
          ...
          if FieldAsInteger('CURRENCY_NO') <> 974 then
          begin
            str := 'Курс ' + FloatToStr(FieldAsFloat('E_RATE')) + #10
                   ...
                   + '='+IntToStr(Round(FieldAsFloat('E_RATE') * FieldAsInteger('ST_SUMMPAY')));
...
procedure TfmMainMenu.frRN01_02Click(Sender: TObject);
var
  PickMonth : TfmPickMonth;
  rep_date : TDateTime;
  v_date: TDate; // Дата отчета
...
begin
  ...
  try
    //Выбираем фамилии руководителей
    if dmGlobal.odsTOPMAN.Active = False then dmGlobal.odsTOPMAN.Open else dmGlobal.odsTOPMAN.Refresh;
    ...
    on E:Exception do
      Application.MessageBox(PChar('Ошибка формирования отчёта:' + #10#13 + E.Message),PChar('Ошибка формирования отчёта'), MB_ICONERROR);
  end;
  ...
  begin
    with frRN01_02 do
    begin
      odsTOPMAN.Locate('KOD','GLING',[loCaseInsensitive]);
      Variables.Items[1].Value := '''' + odsTOPMAN.FieldByName('SNAIM').AsString +  '''';
      Variables.Items[2].Value := '''' + rep_month + ' ' + rep_year + '''';
      odsTOPMAN.Locate('KOD','NACHOTD',[loCaseInsensitive]);
      Variables.Items[3].Value := '''' + odsTOPMAN.FieldByName('SNAIM').AsString + '''';
      PrepareReport();
      ShowPreparedReport;
    end;
  end;
...
procedure TfmMainMenu.RepPayment_1Click(Sender : TObject);
var
  PickYear : TfmPickYear;
  rep_year : string; // Год
  year : Integer;
...
begin
  ...
  with dmGlobal do
  begin
    odsPayment_1.SetVariable('REP_DATE',EncodeDate(year, 1, 1));
    odsPayment_1_2.SetVariable('REP_DATE',EncodeDate(year, 1, 1));
    odsPayment_1.Open;
    odsPayment_1_2.Open;
    frPayment_1.Variables.Items[1].Value := '''' + rep_year + '''';
    frPayment_1.PrepareReport();
    frPayment_1.ShowPreparedReport;
  end;
...
procedure TfmMainMenu.RepRN04Click(Sender : TObject);
var
  v_date_str : string; // Дата отчета в текстовом формате
  rep_date : TDateTime;
  v_date: TDate; // Дата отчета
...
begin
  ...
  with dmGlobal do
  begin
    if odsRN04.Active then odsRN04.Close;
    odsRN04.SetVariable('V_DATE', v_date);
    odsRN04.SetVariable('CURR_AGR', curr_agr);
    odsRN04.Open;
    frRN04.Variables.Items[1].Value := '''' + rep_year + '''';
    frRN04.PrepareReport();
    frRN04.ShowPreparedReport;
  end;
end;

procedure TfmMainMenu.RepPayment2Click(Sender: TObject);
var
  v_date: TDate; // Дата отчета
  v_date_str : string; // Дата отчета в текстовом формате
  month : integer;
...
begin
  ...
  dmGlobal.frPayment_2.PrepareReport();
  dmGlobal.frPayment_2.ShowPreparedReport;

end;

procedure TfmMainMenu.RepRN06Click(Sender: TObject);
var
  v_date: TDate; // Дата отчета
  v_date_str : string; // Дата отчета в текстовом формате
  month : integer;
...
begin
  ...
  dmGlobal.frRN06.PrepareReport();
  dmGlobal.frRN06.ShowPreparedReport;

end;

procedure TfmMainMenu.RepRN08Click(Sender: TObject);
var
  v_date: TDate; // Дата отчета
  v_date_str : string; // Дата отчета в текстовом формате
  month : integer;
  rep_year : integer; // Год
...
begin
  ...
  begin
    ...
    v_date_str := '01.01.' + IntToStr(PickYear.se_Year.Value);
    v_date := StrToDate(v_date_str);
    rep_year := PickYear.se_Year.Value;
  end
  else begin  PickYear.Free; Exit; end;
  PickYear.Free;
  with dmGlobal do
  begin
    if odsTOPMAN.Active = False then odsTOPMAN.Open else odsTOPMAN.Refresh;
    with odsRN08 do
    begin
      if Active then Close;
      SetVariable('YR',rep_year);
      Open;
    end;
    odsTOPMAN.Locate('KOD','GLING',[loCaseInsensitive]);
    frRN08.Variables.Items[1].Value := '''' + odsTOPMAN.FieldByName('SNAIM').AsString +  '''';
    frRN08.Variables.Items[2].Value := '''' + IntToStr(rep_year) + '''';
    odsTOPMAN.Locate('KOD','NACHOTD',[loCaseInsensitive]);
    frRN08.Variables.Items[3].Value := '''' + odsTOPMAN.FieldByName('SNAIM').AsString + '''';
    frRN08.PrepareReport();
    frRN08.ShowPreparedReport;
  end;
end;

procedure TfmMainMenu.RepRN10Click(Sender: TObject);
var
  month, year : Integer;
  v_datefrom_str : string; // Дата отчета в текстовом формате
  v_dateto_str : string;
  year_month_str : string; //Месяц-год для отчёта
  rep_month : string; // Название месяца
  rep_year : string; // Год
  PickMonth : TfmPickMonth;
  v_date : TDateTime;
...
//    v_date := StrToDate(v_date_str);
    rep_month := PickMonth.cbMonth.Items[PickMonth.cbMonth.ItemIndex];
    rep_year := IntToStr(PickMonth.se_Year.Value);
    year_month_str := PickMonth.cbMonth.Text + ' ' + rep_year;
  end
  else begin  PickMonth.Free; Exit; end;
  PickMonth.Free;
  with dmGlobal do
...
//  odsTOPMAN.Locate('KOD','GLING',[loCaseInsensitive]);
    frRN010.Variables.Items[1].Value := '''' + year_month_str +  '''';
//    frRN10.Variables.Items[2].Value := '''' + IntToStr(rep_year) + '''';
//    odsTOPMAN.Locate('KOD','NACHOTD',[loCaseInsensitive]);
//    frRN10.Variables.Items[2].Value := '''' + odsTOPMAN.FieldByName('SNAIM').AsString + '''';
    frRN010.PrepareReport();
    frRN010.ShowPreparedReport;

  end;

end;

procedure TfmMainMenu.RepRN12Click(Sender: TObject);
var
  v_date_str : string; // Дата отчета в текстовом формате
  rep_year : string; // Год
  year : Integer;
...
//  odsTOPMAN.Locate('KOD','GLING',[loCaseInsensitive]);
    frRN010.Variables.Items[1].Value := '''' + rep_year +  '''';
//    frRN10.Variables.Items[2].Value := '''' + IntToStr(rep_year) + '''';
//    odsTOPMAN.Locate('KOD','NACHOTD',[loCaseInsensitive]);
//    frRN10.Variables.Items[2].Value := '''' + odsTOPMAN.FieldByName('SNAIM').AsString + '''';
    frRN010.PrepareReport();
...
procedure TfmMainMenu.RepRN13Click(Sender: TObject);
begin
  with dmGlobal do
  begin
    with odsRN013 do
    ...
    frRN013.PrepareReport();
    frRN013.ShowPreparedReport;
  end;
end;

procedure TfmMainMenu.RepRN14Click(Sender: TObject);
var
  month, year : Integer;
  v_datefrom_str : string; // Дата отчета в текстовом формате
  v_dateto_str : string;
...
begin
  ...
  begin
    ...
    frRN014.PrepareReport();
    frRN014.ShowPreparedReport;
  end;
end;

procedure TfmMainMenu.RepRN16Click(Sender: TObject);
var
  v_date_str : string; // Дата отчета в текстовом формате
  rep_year : string; // Год
  year : Integer;
...
begin
  ...
  odsTOPMAN.Locate('KOD','GENDIR',[loCaseInsensitive]);
    frRN16.Variables.Items[1].Value := '''' + odsTOPMAN.FieldByName('SNAIM').AsString +  '''';
    frRN16.Variables.Items[2].Value := '''' + rep_year + '''';
//    odsTOPMAN.Locate('KOD','NACHOTD',[loCaseInsensitive]);
//    frRN10.Variables.Items[2].Value := '''' + odsTOPMAN.FieldByName('SNAIM').AsString + '''';
    frRN16.PrepareReport();
    frRN16.ShowPreparedReport;
...
procedure TfmMainMenu.RepRN20Click(Sender: TObject);
var
  v_date: TDate; // Дата отчета
  v_date_str : string; // Дата отчета в текстовом формате
  month : integer;
...
begin
  ...
  begin
    ...
    begin
      if Active then Close;
      SetVariable('YEAR',rep_year);
      Open;
    end;
    odsTOPMAN.Locate('KOD','GLING',[loCaseInsensitive]);
    frRN020.Variables.Items[1].Value := '''' + odsTOPMAN.FieldByName('SNAIM').AsString +  '''';
    frRN020.Variables.Items[2].Value := '''' + IntToStr(rep_year) + '''';
    odsTOPMAN.Locate('KOD','NACHOTD',[loCaseInsensitive]);
    frRN020.Variables.Items[3].Value := '''' + odsTOPMAN.FieldByName('SNAIM').AsString + '''';
    frRN020.PrepareReport();
    frRN020.ShowPreparedReport;
  end;
end;

procedure TfmMainMenu.RepRN22Click(Sender: TObject);
var
  v_date: TDate; // Дата отчета
  v_date_str : string; // Дата отчета в текстовом формате
  month : integer;
...
//    if odsTOPMAN.Active = False then odsTOPMAN.Open else odsTOPMAN.Refresh;
    ...
    begin
      if Active then Close;
      SetVariable('YEAR',EncodeDate(rep_year,1,1));
      Open;
    end;
//  odsTOPMAN.Locate('KOD','GLING',[loCaseInsensitive]);
    frRN022.Variables.Items[1].Value := '''' + IntToStr(rep_year) +  '''';
//  frRN020.Variables.Items[2].Value := '''' + IntToStr(rep_year) + '''';
//  odsTOPMAN.Locate('KOD','NACHOTD',[loCaseInsensitive]);
//  frRN020.Variables.Items[3].Value := '''' + odsTOPMAN.FieldByName('SNAIM').AsString + '''';

    frRN022.PrepareReport();
    frRN022.ShowPreparedReport;
  end;
end;

procedure TfmMainMenu.RepRN18Click(Sender:TObject);
var
  v_from_date_str : String; // Дата начала временого интрвала отчета в текстовом формате
  v_to_date_str   : String; // Дата конча временого интрвала отчета в текстовом формате
...
begin
  ...
  begin
    ...
    begin
      if Active then Close;
      SetVariable('DATEFROM',EncodeDate(rep_from_year,1,1));
      SetVariable('DATETO',EncodeDate(rep_to_year,1,1));
      Open;
    end;
    odsTOPMAN.Locate('KOD','GLING',[loCaseInsensitive]);
    frRN018.Variables.Items[1].Value := '''' + odsTOPMAN.FieldByName('SNAIM').AsString +  '''';
    frRN018.Variables.Items[2].Value := '''' + IntToStr(rep_from_year) + '''';
    frRN018.Variables.Items[3].Value := '''' + IntToStr(rep_to_year) + '''';
//  odsTOPMAN.Locate('KOD','NACHOTD',[loCaseInsensitive]);
//  frRN020.Variables.Items[3].Value := '''' + odsTOPMAN.FieldByName('SNAIM').AsString + '''';
    frRN018.PrepareReport();
    frRN018.ShowPreparedReport;
  end;
end;
